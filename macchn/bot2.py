...
BOT_TOKEN = "MTQzMTg0ODgzODQxMDUzOTAyMA.GlW6VP.I__rVatmNrazUHi2vYmJ4Yzy2dxaGd8QZkH-rE"
BOT_NAME = "Bot2"
...
def save_state():
    try:
        with open(STATE_FILE, 'w') as f:
            json.dump({'run_24_7_data': run_24_7_data, 'instance_data': instance_data, 'timestamp': time.time()}, f, indent=2)
    except: pass
...
async def run_24_7_task(user_id, instance_name, session_id, script_link, pip_packages, cookies, channel):
    ...
    while True:
        try:
            cycle += 1
            run_24_7_data[user_id][instance_name] = {"session_id": session_id, "script_link": script_link, "cycle": cycle, "last_refresh": time.time()}
            save_state()
            ...
            commands = [
                f"curl -o script.py {script_link}",
                "sudo dnf install -y python3 python3-pip"
            ]
            for pkg in pip_packages:
                commands.append(f"pip3 install {pkg}")
